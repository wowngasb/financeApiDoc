...
json::value HttpApiPost(wstring module, wstring api, json::value params)
{
	...
	wcscat_s(query_path, L"/api/");
	wcscat_s(query_path, module.c_str());
	wcscat_s(query_path, L"/");
	wcscat_s(query_path, api.c_str());

	wchar_t auth_str[256];
	wcscat_s(auth_str, L"dyyadmin:");
	wcscat_s(auth_str, api_key);
